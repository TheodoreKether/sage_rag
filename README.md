...
| 脚本 | 用途 |
|------|------|
| `build_qa_dataset.py` | Step 5：构建 QA 数据集 |
| `test_dense_retriever.py` | Step 6：Dense 检索 smoke test |
| `evaluate_dense.py` | Step 7：Dense 检索全量评测 |
...
| `src/generation/` | QA 数据集生成 |
| `src/retrieval/` | Dense Retriever（`RetrieverBase` 接口） |
| `src/evaluation/` | 检索评测（指标 / 报告 / CLI） |
...
| Dense 检索基线 | `src/retrieval/` | ✅ 已完成 |
| 检索评测 | `src/evaluation/` | ✅ 已完成 |
| BM25 / Hybrid / Graph-RAG | — | ⏳ 待实现 |
| LLM 生成 / 端到端 RAG 评测 | — | ⏳ 待实现 |
...
### 阶段 5：Dense 检索基线（`src/retrieval/`）

**输入：** 用户 query + `data/vector_store/`  
**输出：** Top-K Evidence Units（含 rank、score、unit_id、text 等）

基于 `BAAI/bge-m3` 对 query 编码，在 FAISS `IndexFlatIP` 上做相似度搜索。所有后续检索方法（BM25、Hybrid、SAGE-RAG）实现统一的 `RetrieverBase.retrieve(query, top_k)` 接口，便于公平对比。

### 阶段 6：检索评测（`src/evaluation/`）

**输入：** `qa_pairs.jsonl`（ground truth 为 `supporting_evidence.unit_id`）+ 任意 Retriever  
**输出：** `results/retrieval_results.jsonl` + `results/retrieval_dense_report.md`

对每条 QA 问题执行检索，计算 Recall@1/5/10、MRR、nDCG@5/10。每条结果保存完整 Top-10 排序及分数，便于 case study 和跨方法对比。指标计算与 retriever 实现解耦，换 retriever 不改评测逻辑。

**当前 Dense 基线（878 条 QA，BGE-M3 + FAISS）：**
...
> 指标偏低主要因 QA 由 placeholder LLM 按模板生成，问题与 gold unit 语义弱对齐；definition 题型表现最好（R@10 ≈ 15.5%）。自然语言 query（如「工业机器人数据交换模型有哪些要求」）检索效果显著更好。
...
├── retrieval_results.jsonl         # 可选，检索评测逐题结果
├── retrieval_dense_report.md       # 可选，Dense 检索汇总报告
└── figures/
```
...
### Step 6 — Dense 检索（手动测试）

加载 FAISS 索引，对单条 query 或 QA 数据集抽样做 smoke test。结果输出到终端。

```bash
python test_dense_retriever.py \
    --index data/vector_store \
    --query "工业机器人数据交换模型有哪些要求" \
    --top-k 5
```
...
```powershell
python test_dense_retriever.py --index data/vector_store --qa-file data/qa_dataset/qa_pairs.jsonl --num-queries 5 --top-k 5 --device cuda
```

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--index` | `data/vector_store` | 向量库目录 |
| `--query` | 无 | 单条测试 query |
| `--qa-file` | 无 | 从 QA JSONL 抽样测试 |
| `--top-k` | `5` | 返回条数 |
...
在完整 QA 数据集上评测 Dense Retriever，输出逐题结果与汇总报告。

**调试（随机 100 条）：**

```bash
python src/evaluation/evaluate_dense.py \
    --qa data/qa_dataset/qa_pairs.jsonl \
    --index data/vector_store \
    --top-k 10 \
    --sample 100 \
    --device cuda
...
```powershell
python evaluate_dense.py --qa data/qa_dataset/qa_pairs.jsonl --index data/vector_store --top-k 10 --device cuda
```

**输入：** `qa_pairs.jsonl` + `vector_store/`  
**输出：**

| 文件 | 说明 |
|------|------|
| `results/retrieval_results.jsonl` | 每题 Top-K 检索结果（含 rank、unit_id、score）及逐题指标 |
| `results/retrieval_dense_report.md` | 平均 Recall@K、MRR、nDCG、耗时等 |

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--qa` | `data/qa_dataset/qa_pairs.jsonl` | QA 数据集路径 |
| `--index` | `data/vector_store` | 向量库目录 |
| `--top-k` | `10` | 每题检索条数 |
| `--sample` | 无 | 随机抽样 N 条（调试用） |
| `--output` | `results/retrieval_results.jsonl` | 逐题结果输出 |
| `--report` | `results/retrieval_dense_report.md` | 汇总报告输出 |
...
# 7. Dense 检索评测（需 vector_store + qa_dataset）
python evaluate_dense.py --qa data/qa_dataset/qa_pairs.jsonl --index data/vector_store --top-k 10 --device cuda
```
...
data/qa_dataset/qa_pairs.jsonl
        ...
        │
        ▼  Step 6  test_dense_retriever.py（手动 smoke test）
        │
        ▼  Step 7  evaluate_dense.py
results/retrieval_results.jsonl
        │
        └──▶ results/retrieval_dense_report.md
```
...
| Dense Recall@1（全量评测） | 0.80% |
| Dense Recall@5 | 2.39% |
| Dense Recall@10 | 4.44% |
| Dense MRR | 0.0164 |
...
**Q: 只想跑检索评测？**  
A: 需 `data/vector_store/` 和 `data/qa_dataset/qa_pairs.jsonl`，直接运行 Step 7 即可。

**Q: 只想重建向量索引？**  
A: 只需 `data/evidence_units/evidence_units.jsonl`，直接运行 Step 4 即可。向量数应与 JSONL 有效行数一致。

**Q: Dense 检索 Recall 为什么很低？**  
A: 当前 QA 由 placeholder LLM 按模板生成（如「条款 X 规定了哪些要求」），与 gold Evidence Unit 语义弱对齐；13 份异构标准混检也会引入 cross-document 干扰。definition 题型且含章节标题的问题检索效果更好。自然语言 query 的 smoke test 表现正常。后续 BM25 / Hybrid 可在同一评测框架下对比。
...
**Q: 模型已缓存但仍尝试联网？**  
A: PowerShell 下可设 `$env:HF_HUB_OFFLINE = "1"` 后运行 embedding / retrieval 相关命令。
